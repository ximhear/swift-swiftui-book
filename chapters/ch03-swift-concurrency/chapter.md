...
> **Note**: 본문 코드 블록은 핵심을 부각하기 위해 일부 세부 구현(검증 로직, 제네릭 파라미터 등)을 간소화한 발췌입니다. 전체로 컴파일·실행 가능한 버전은 이 장의 `examples/` 디렉터리(`ActorPatterns.swift`, `StructuredConcurrency.swift`, `TaskGroupPatterns.swift` 등)에 있습니다. 본문과 예제 파일에서 같은 개념이 약간 다른 형태로 보이더라도 의도된 단순화입니다.
...
암시적 Sendable 추론도 이해해야 합니다. **비-public struct와 enum**(앱 모듈의 일반 타입 포함)은 모든 저장 프로퍼티(또는 associated value)가 Sendable이면 명시적 선언 없이도 **암시적으로 Sendable**로 추론됩니다. public 타입이 Library Evolution 모드에서 같은 추론을 받으려면 `@frozen`이 필요합니다(아래 Note 참고):
...
> **Note**: `@frozen`은 타입의 메모리 레이아웃 변경 불가를 약속하는 어트리뷰트로, 묵시적 Sendable 추론과 깊은 관련이 있습니다.
>
> `@frozen`은 **타입의 메모리 레이아웃이 미래에도 변경되지 않을 것**을 약속하는 어트리뷰트입니다. struct에서는 "저장 프로퍼티가 추가되지 않는다", enum에서는 "케이스가 추가/변경되지 않는다"는 약속이고, 컴파일러는 이 정보를 활용해 직접 필드 접근·인라이닝·exhaustive switch 같은 최적화를 수행합니다.
>
> 이 어트리뷰트는 주로 **Library Evolution이 활성화된 모듈**(Swift 표준 라이브러리, Apple 시스템 프레임워크, 바이너리로 배포되는 XCFramework 등)에서 의미를 가집니다. Library Evolution 모드에서는 public 타입의 레이아웃이 미래에 바뀔 수 있다고 가정하므로, 클라이언트가 정적 가정을 하려면 `@frozen`이 필요합니다.
>
> **일반 앱 모듈에서는 Library Evolution이 꺼져 있어 모든 타입이 사실상 frozen처럼 동작**합니다. 따라서 앱 코드의 `struct Point { ... }`는 `@frozen` 없이도 묵시적 Sendable 추론을 받습니다. 위 문장의 "frozen struct"는 정확히는 **"외부에 공개된 라이브러리에서 묵시적 Sendable을 얻으려면 `@frozen`이 필요하다"** 는 의미입니다. 표준 라이브러리의 `Int`, `Bool`, `Array`, `Optional` 등이 모두 `@frozen`으로 선언된 이유가 바로 이것입니다.
...
빌더로 객체를 조립한 후 actor에 핸드오프하는 패턴은 Region-based Isolation의 대표적인 활용처입니다. 폼 제출 시나리오를 예로 들어봅시다.

```swift
import Foundation

// Sendable이 아닌 빌더 — 가변 상태를 가짐
...
`FormBuilder`는 `Sendable`도 `@unchecked Sendable`도 아닌 평범한 클래스입니다. 그럼에도 위 세 가지 패턴 모두 컴파일을 통과합니다 — 컴파일러가 "form 변수가 actor에 넘어간 뒤로는 호출자가 더 이상 참조하지 않는다"는 사실을 region 분석으로 증명하기 때문입니다.

반면 actor에 넘긴 뒤 같은 변수를 다시 사용하면 region이 겹쳐 컴파일 에러가 발생합니다:

```swift
let form = FormBuilder()
form.setField("name", "Alice")
await service.submit(form)
print(form.fields)  // ❌ error: sending 'form' risks causing data races
                    //    note: access can happen concurrently
```

이 패턴은 실무에서 다음 시나리오에 자주 등장합니다:
- **Form/Request 빌더**: 복잡한 요청 객체를 조립 후 네트워크 actor에 위임
- **렌더링 파이프라인**: `CGImage`·`CIImage` 같은 non-Sendable 객체를 합성 후 `@MainActor`로 전달
- **DB 모델 객체**: ORM 모델을 만들고 persistence actor에 저장
- **파이프라인 단계**: 함수 A가 객체를 만들고 함수 B가 받아 변환한 뒤 actor로 보냄

Swift 5에서는 이 모든 경우에 `Sendable` 채택이 강제됐지만, Swift 6의 region 분석 덕분에 **객체 구조를 동시성 요건에 맞춰 왜곡할 필요가 없어졌습니다**.
...
"부모 컨텍스트를 상속한다"는 말은 추상적이라 혼란스러울 수 있습니다. 구체적으로 `Task { }`가 부모로부터 가져오는 항목은 세 가지입니다.

**1. Actor 격리(isolation)** — 가장 중요

`Task { }`가 생성되는 시점의 **호출자 격리(caller's isolation)**가 그대로 자식 Task의 격리가 됩니다. 즉:

| 호출 위치 | `Task { }` 본문의 격리 |
|---|---|
| `@MainActor` 메서드 내부 | `@MainActor` 격리 |
| 일반 `actor MyActor` 메서드 내부 | `MyActor` 격리 |
| `nonisolated` 함수 내부 | 격리 없음 |
...
class MainView {
    ...
    func runChildTask() {
        // 이 메서드는 MainActor 격리
        Task {
            // ✅ 부모(MainActor) 격리를 상속 → self.value에 직접 접근 가능
            self.value = "updated"
        }
        ...
        Task.detached {
            ...
            // self.value = "X"
            // 명시적으로 hop 필요:
            // await MainActor.run { self.value = "X" }
        }
    }
}
...
actor Counter {
    ...
    func bumpInChild() {
        Task {
            // ✅ Counter 격리 상속 → self.n에 직접 접근 가능
            self.n += 1
        }
    }
}
...
이 차이가 결정적으로 중요한 이유는 **race condition 방지**입니다. MainActor 메서드에서 `Task { }`를 만들면 그 Task 본문도 메인 스레드(=MainActor)에서 실행되므로 UI 상태를 안전하게 변경할 수 있습니다. 반면 `Task.detached`는 어떤 스레드에서든 실행될 수 있어 `self.value`에 직접 접근하면 컴파일러가 차단합니다.

**2. 우선순위(priority)**

`Task { }`는 호출자의 현재 Task 우선순위를 그대로 상속합니다. 예를 들어 `userInitiated` 우선순위의 Task 안에서 `Task { }`를 만들면 자식도 `userInitiated`로 시작합니다.

```swift
Task(priority: .userInitiated) {
    print(Task.currentPriority)  // high (= userInitiated, 같은 값의 별칭)

    Task {
        print(Task.currentPriority)  // high (상속됨)
    }

    Task.detached {
        print(Task.currentPriority)  // medium (시스템 기본값)
    }
}
```

명시적으로 다른 우선순위를 지정하면 부모를 덮어쓸 수 있습니다: `Task(priority: .background) { ... }`.

**3. TaskLocal 값**

`@TaskLocal`로 선언된 값은 부모 Task의 동적 컨텍스트로 자식 Task에 전파됩니다. 로깅 컨텍스트, 트랜잭션 ID, 사용자 추적 정보 등에 유용합니다.
...
Trace.$requestID.withValue("req-123") {
    Task {
        print(Trace.requestID)  // "req-123" (상속됨)
    }

    Task.detached {
        print(Trace.requestID)  // "none" (상속 안 됨)
    }
}
```

### Task vs Task.detached — 비교 표

🟢 기본

| 항목 | `Task { }` | `Task.detached { }` |
|---|---|---|
| Actor 격리 | 부모로부터 상속 | 상속 없음 (격리되지 않은 컨텍스트로 시작) |
| 우선순위 | 부모 상속 | 시스템 기본값 (`medium`) |
| TaskLocal | 부모 값 전파 | 새 컨텍스트 시작 |
| 부모-자식 관계 | 비구조적 (논리적 부모는 있지만 cancellation 자동 전파 없음) | 완전 독립 |
| 일반적인 사용처 | UI 이벤트에 반응해 백그라운드 작업 시작 | 격리에서 의도적으로 빠져나와 백그라운드 처리 |

### 흔히 만나는 함정

**함정 1: SwiftUI의 `Button { }` 액션에서 `Task { }`**
...
struct ContentView: View {
    ...
    var body: some View {
        Button("Load") {
            // 이 클로저는 MainActor 격리 → Task { }도 MainActor 격리
            Task {
                let fetched = try await api.fetch()  // ← await 동안 스레드 hop 가능
                items = fetched  // ✅ MainActor라 안전
            }
        }
    }
}
...
`api.fetch()`가 `await` 동안 백그라운드로 hop했다가, 결과를 받을 때 다시 MainActor로 돌아옵니다. `items = fetched`가 항상 안전한 이유입니다.

**함정 2: actor 메서드 안에서 `Task.detached`를 쓰면 격리가 끊김**
...
actor Cache {
    ...
    func warmUp() {
        Task.detached {
            // self.data 접근 시 await + 명시적 hop 필요
            await self.data["x"] = ...   // ❌ 단순 접근 안 됨
        }
    }
}
```

대부분의 경우 `Task { }`가 답입니다. `Task.detached`는 "이 작업은 부모와 완전히 독립적이어야 한다"는 강한 의도 표현으로만 사용하세요.
...
```swift
// 주의: 반환 타입을 stdlib의 제네릭 `Result`로 두면 인자 없이 쓸 수 없어
// 컴파일 에러가 납니다. 도메인 결과 타입은 별도 이름(ProcessResult)을 씁니다.
...
class ImageProcessor: ObservableObject {
    ...
    nonisolated func performHeavyProcessing(
        ...
        // CPU 집약적 작업
        // nonisolated async 함수는 호출자의 actor 격리를 벗어나
        // 전역 cooperative thread pool(generic executor)에서 실행됨(SE-0338).
        // 따라서 별도 Task.detached 없이도 메인 스레드를 차단하지 않음
        await applyFilters(image)
    }
}
```

여기서 핵심은 **`nonisolated` async 함수의 실행 위치**입니다. SE-0338에 따라, `@MainActor` 메서드에서 `nonisolated` async 함수를 `await`로 호출하면 그 함수 본문은 호출자(MainActor)의 executor가 아니라 **전역 cooperative thread pool(generic executor)로 hop하여 실행**됩니다. 즉 `performHeavyProcessing`은 호출 시점에 이미 메인 스레드를 벗어나므로, `Task.detached`로 별도 백그라운드를 지정하지 않아도 UI가 프리징되지 않습니다. "백그라운드 실행을 위해 `Task.detached`가 필요하다"는 흔한 오해와 달리, `nonisolated`만으로 격리에서 벗어나는 것입니다.

> **Note**: Swift 6.2에서 도입된 `nonisolated(nonsending)`(SE-0461)는 `nonisolated` async 함수가 호출자의 격리를 따라가도록 바꾸는 **opt-in 기능**입니다. 위 설명은 Swift 6.1 기준의 기본 동작이며, 6.2 이후에도 별도 지정 없이는 동일하게 전역 executor에서 실행됩니다.
...
- **우선순위(Priority)**: 실제 distinct 레벨은 4개입니다 — `.high`(=`.userInitiated`, rawValue 25), `.medium`(21), `.low`(=`.utility`, 17), `.background`(9). `.high`와 `.userInitiated`, `.low`와 `.utility`는 각각 같은 값의 별칭(alias)입니다.
...
`@TaskLocal`은 **Task 계층 전체에 암시적으로 전파되는 컨텍스트 값**입니다. 함수 시그니처에 매개변수를 추가하지 않고도 호출 트리 전체에 "현재 요청은 무엇이고 사용자는 누구다"와 같은 부수 정보를 흘려보낼 수 있습니다.

#### 왜 필요한가 — ThreadLocal과 비교

전통적인 `ThreadLocal`(NSThread.dictionary, pthread_key 등)은 스레드 단위로 값을 저장합니다. 그러나 Swift Concurrency에서는 하나의 Task가 `await` 마다 다른 스레드로 옮겨다닙니다. ThreadLocal을 그대로 쓰면 `await` 전후로 값이 사라지거나 다른 요청의 값을 보게 되는 치명적 버그가 발생합니다.

| 항목 | ThreadLocal | TaskLocal |
|---|---|---|
| 저장 단위 | 스레드 | Task |
| `await` 후 값 보존 | ❌ 깨짐 | ✅ 유지 |
| 자식 Task로 전파 | ❌ | ✅ 자동 |
| 범위 제한 | 명시적 해제 필요 | `withValue` 블록 종료 시 자동 복원 |
| 스레드 안전성 | 스레드별 격리 | Task별 격리 |

`@TaskLocal`은 Swift Concurrency의 의미론에 맞춰 설계된 ThreadLocal의 대체재입니다.

#### 멘탈 모델 — "스코프 바인딩"

`@TaskLocal` 변수는 **읽기 전용**입니다. 직접 대입할 수 없고, 반드시 `withValue` 블록을 통해 **일시적으로 값을 바인딩**합니다. 블록 안에서는 새 값이 보이고, 블록을 빠져나오면 이전 값으로 복원됩니다.
...
print(Trace.requestID)                  // "default"
await Trace.$requestID.withValue("A") {
    print(Trace.requestID)              // "A"
    await Trace.$requestID.withValue("B") {
        print(Trace.requestID)          // "B" (안쪽이 바깥을 덮어씀)
    }
    print(Trace.requestID)              // "A" (블록 종료 시 복원)
}
print(Trace.requestID)                  // "default"
```

문법적 포인트:
- 선언: `@TaskLocal static var name: T = default` — 반드시 `static`이고 기본값 필요
- 읽기: `Trace.requestID` (점 접근)
- 쓰기: `Trace.$requestID.withValue(...) { ... }` — `$` prefix로 wrapper 접근

#### 자식 Task로 자동 전파

`withValue` 블록 안에서 만들어진 모든 Task — `async let`, `TaskGroup`, `Task { }` — 는 부모의 TaskLocal 값을 그대로 상속합니다.

```swift
await Trace.$requestID.withValue("req-B") {
...
단, **`Task.detached`는 상속하지 않습니다** — 부모 Task와 의도적으로 끊긴 새 컨텍스트로 시작합니다.

```swift
await Trace.$requestID.withValue("req-C") {
    await Task.detached {
        log("detached")
        // → [default] detached   ← 상속 안 됨
    }.value
}
```
...
서버나 백엔드 코드에서 매 요청을 추적할 때 가장 자연스러운 활용입니다. 함수 시그니처에 `requestID`를 매개변수로 끌고 다닐 필요가 없습니다.
...
func processOrder() async {
    log("주문 처리 중")          // 매개변수 없이도 컨텍스트 자동 사용
}
...
#### 실전 패턴 2 — 테스트에서 의존성 모킹

`@TaskLocal`은 **테스트 코드에서 의존성을 깔끔하게 교체**하는 방법으로도 유용합니다. 함수에 dependency injection 매개변수를 추가하지 않고, 호출자가 컨텍스트만 갈아끼우면 됩니다.
...
func isExpired(_ token: Token) -> Bool {
    token.expiresAt < Env.now()    // 현재 시간 의존성
}
...
Env.$now.withValue({ fixed }) {
    XCTAssertTrue(isExpired(token))
}
```

기존의 protocol + 생성자 주입 방식보다 호출 지점이 가볍습니다. 다만 너무 남용하면 의존성 흐름이 보이지 않게 되므로, **순수 유틸리티 함수의 환경값 주입** 정도로 한정하는 것이 좋습니다.

#### 실전 패턴 3 — 분산 추적(distributed tracing)

OpenTelemetry나 자체 트레이싱 시스템에 spanID를 자동 전파할 때 표준 패턴입니다.
...
func withSpan<T>(_ name: String, _ body: () async throws -> T) async rethrows -> T {
    ...
    return try await Tracing.$spanID.withValue(spanID) {
        let start = Date()
        defer {
            let duration = Date().timeIntervalSince(start)
            recordSpan(name: name, id: spanID, duration: duration)
        }
...
// 사용
await withSpan("handleOrder") {
    await withSpan("validate") {
        // Tracing.spanID는 자동으로 "validate" 스팬의 ID
    }
    await withSpan("persist") {
        // 다른 스팬 ID
    }
}
```
...
```swift
@TaskLocal static var x: Int = 0
Trace.x = 99  // ❌ 컴파일 에러: get-only 프로퍼티
```

`@TaskLocal`은 읽기 전용입니다. 값을 바꾸려면 반드시 `withValue` 블록을 사용해야 합니다.

**함정 2: 블록 밖에서 값이 보일 거라고 기대**

```swift
await Trace.$requestID.withValue("A") {}
print(Trace.requestID)   // "default" — withValue를 빠져나오는 즉시 복원됨
```

withValue는 스코프 한정이지 영구 설정이 아닙니다.

**함정 3: `Task.detached`로 끊기**
...
분리된 Task에서도 컨텍스트가 필요하면 값을 명시적으로 캡처하거나, `Task { }`(상속됨)를 사용하세요.

**함정 4: 값 타입과 참조 타입의 의미 차이**

`@TaskLocal`에 클래스 인스턴스를 저장하면 모든 Task가 **같은 인스턴스**를 공유합니다. 만약 그 인스턴스가 가변 상태를 갖고 있다면 데이터 레이스가 발생할 수 있습니다. **불변 값 타입(struct) 또는 Sendable 타입**을 권장합니다.

#### 정리

| 특성 | 동작 |
|---|---|
| 전파 | 부모 Task → 자식 Task로 자동 (Task.detached 제외) |
| 범위 | `withValue` 블록 안 → 블록 종료 시 이전 값으로 복원 |
| 가변성 | 읽기 전용. `withValue`로 임시 바인딩만 가능 |
| 스레드 안전 | Task별 격리. `await`로 스레드가 바뀌어도 값 유지 |
| 적합한 용도 | 로깅 컨텍스트, 트랜잭션 ID, 트레이싱, 환경값 주입 |
| 부적합한 용도 | 가변 상태 공유, 영구 설정, 비즈니스 로직 의존성 흐름 |
