...
> Swift의 타입 시스템은 단순히 컴파일 에러를 잡아주는 도구가 아닙니다. 값 타입과 참조 타입의 선택, 프로토콜의 존재적 타입과 불투명 타입의 차이 — 이 결정들이 앱의 성능과 아키텍처를 근본적으로 좌우합니다. 이 장에서는 Swift 타입 시스템의 내부 동작을 이해하고, 실무에서 올바른 선택을 내릴 수 있는 기준을 세웁니다.
...
├─────────────────────────────────┤
│ Value Witness Table 포인터       │ ← 복사, 이동, 해제 방법
├─────────────────────────────────┤
...
```swift
// Cat: name(String) — String은 내부적으로 참조 타입이므로
// 포인터 크기만 차지 → 인라인 저장 가능
...
> **Tip**: 프로토콜 타입으로 빈번하게 사용되는 구조체는 24바이트 이하로 유지하면 힙 할당을 피할 수 있습니다. `MemoryLayout<T>.size`로 미리 확인하는 습관을 들이세요.

### Value Witness Table(VWT)

🔴 고급

Existential Container의 네 번째 word는 **Value Witness Table(VWT)** 포인터입니다. VWT는 타입의 **생명주기 관리**를 담당하는 함수 테이블로, 다음 연산들의 구현을 포함합니다.
