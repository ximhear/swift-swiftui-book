...
> 제네릭(Generic)과 프로토콜(Protocol)은 Swift 타입 시스템의 양대 축입니다. 기본적인 사용법을 넘어, 연관 타입(Associated Type)의 제약 조건 설계, Type Erasure의 실전 구현, 그리고 Swift 5.7 이후 등장한 Primary Associated Type까지 — 이 장에서는 유연하면서도 타입 안전한 추상화를 설계하는 고급 기법을 다룹니다.
...
> **Note**: 이것은 제네릭 문법과 비슷해 보이지만, 의미가 다릅니다. 제네릭 `<T>`는 호출자가 타입을 지정합니다. 반면 Primary Associated Type에서 연관 타입의 실제 값을 결정하는 주체는 여전히 **채택 타입**입니다. 꺾쇠 괄호는 `some Container<String>`처럼 **사용처에서 그 연관 타입을 특정 타입으로 제약(constraint)**하는 문법입니다 — 채택자가 정한 연관 타입이 사용처가 요구하는 타입과 일치하는지 컴파일러가 검증합니다.
...
1장에서 살펴봤듯이, `any` 키워드로 존재적 타입(Existential Type)을 사용할 수 있지만 한계가 있습니다:
...
> **Note**: `any DataSource<User>`는 내부적으로 존재적 타입이므로 동적 디스패치 비용이 있습니다. 성능이 중요한 경로에서는 제네릭이나 `some`을 사용하세요. 하지만 대부분의 앱 로직에서 이 비용은 무시할 수 있는 수준입니다.
...
`some` 키워드를 반환 타입에 사용하는 불투명 반환 타입(Opaque Return Type)은 1장에서 기본 개념을 살펴봤습니다. 이 절에서는 실무에서의 활용 전략과 주의점을 깊이 다룹니다.
...
> **Warning**: `some`은 "구체 타입을 숨길 뿐, 단 하나의 구체 타입으로 고정"하는 키워드입니다. 따라서 분기마다 다른 타입을 반환하면 컴파일 에러가 납니다. 런타임 조건에 따라 서로 다른 타입을 돌려줘야 한다면 `some`이 아니라 `any`를 사용하거나, 위 `AdaptiveTheme`처럼 하나의 타입으로 통합하세요.
...
```swift
// 이 두 함수는 거의 동등하다 (opaque parameter는 제네릭의 설탕 문법)
...
다만 완전히 같지는 않습니다. 제네릭 형태는 타입 매개변수 `C`에 이름이 있어 `where` 절이나 본문에서 다시 참조할 수 있고, 호출부에서 `process(...)`에 타입 인자를 명시할 수도 있습니다. opaque parameter는 그 타입을 가리킬 이름이 없으므로 이런 활용이 불가능합니다. 따라서 opaque parameter는 "타입 매개변수를 다시 참조할 필요가 없을 때" 쓰는 간결한 표현으로 보면 됩니다.

그리고 **제네릭이 반드시 필요한 상황**이 있습니다:
...
> **Warning**: 매개변수에 쓰인 `some`은 매개변수마다 **독립적인 타입 매개변수**로 풀립니다. 즉 `func merge(_ a: some Collection<Int>, _ b: some Collection<Int>)`에서 `a`와 `b`는 서로 다른 타입일 수 있습니다. "두 인자가 같은 타입이어야 한다"는 제약은 `some`으로 표현할 수 없으므로, 그럴 때는 이름 있는 제네릭 매개변수 `<C>`를 써야 합니다.
...
### 함수마다 독립적인 Opaque Return Type

🔴 고급

서로 다른 함수가 각각 `some P`를 반환하면, 두 반환 타입은 **서로 독립적인 구체 타입**으로 취급됩니다. 이는 특정 버전에서 새로 생긴 기능이 아니라, 불투명 반환 타입이 처음 도입된 Swift 5.1부터 항상 성립하는 기본 성질입니다.

```swift
// 예제에서 직접 정의한 프로토콜입니다.
// (주의: SwiftUI의 Animation은 프로토콜이 아니라 구조체이므로
//  some Animation은 컴파일되지 않습니다. 혼동을 피하려고
//  여기서는 Effect라는 별도 프로토콜을 씁니다.)
protocol Effect {
    func apply()
}
...
> **Note**: 여기서 말하는 "함수마다 독립적인 opaque 타입"은, 하나의 시그니처 안에서 여러 opaque를 쓰는 **구조적 불투명 반환 타입(structural opaque result types, SE-0328)**과는 다른 개념입니다. 후자는 `-> (some P, some Q)`처럼 한 반환 위치에 여러 `some`을 두는 기능으로 Swift 5.7에서 도입되었으며, 위 예제와는 형태가 다릅니다.
